---
name: frontend-developer-tech-challenge
...
- **Styling**: SCSS (no UI libraries)
- **State**: `zustand`
- **Forms**: `react-hook-form` + `zod`
- **Translations**: `react-i18next` (Contextual, NO hardcoded strings. EN + RU)
- **Quality check**: Prettier, ESLint and Stylelint
- **Coding rules**: Use DRY, KISS and SOLID.
...
When invoked by the `/ralph` loop (or manually) to build a specific feature, follow this strict step-by-step Atomic implementation path (Do NOT use FSD):

1. **Understand Context**: Read `README.md` or review existing architecture.
2. **Create Atomic Structure**: Features should be flat or structured by page/domain in `src/`. Inside a domain/page folder, use standard atomic layers (`ui/`, `components/`, `widgets/`).
3. **Use index.ts for Re-exports**: Every folder (`ui/`, `components/`, `hooks/`, `utils/`, `store/`, `types/`) MUST have an `index.ts` returning its public exports.
4. **Implement Store**: Store should be kept per-feature or per-page, placed alongside its domain logic (`store/index.ts`).
5. **Implement API**: Create mock data or real endpoints in `api/`.
6. **Create Hooks & Utils**: Keep `hooks/` and `utils/` close to the components that need them, re-exported via `index.ts`.
7. **Add Translations**: Use the `contextual-translations` skill to write Russian/English keys in `locales/`. NO hardcoded strings.
8. **Build UI & Widgets**: Use `ui-ux-pro-max` to design and build `ui/` (atoms/molecules), `components/` (organisms), and `widgets/` (templates).
9. **Create Page**: Wire everything together in `page.tsx`.
10. **Validation & Code Review**: Validate Code Quality scripts compliance.
